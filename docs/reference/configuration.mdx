...
| `DOORMAN_API_KEY` | API key for doorman.com (agent API mode) | — |
| `DOORMAN_API_URL` | Backend endpoint override | `https://api.doorman.com` |
| `DOORMAN_LOCAL_MODE` | Set `true` to disable all API calls | `false` |
| `CHECK_INTERVAL` | Seconds between collection cycles (agent mode) | `30` |
| `DOORMAN_SANITIZE_TASK_SIGNATURES` | Set `false` to disable task name sanitization | `true` |
...
thresholds:
  ...
  # Requires DoormanStampPlugin for accurate measurement.
...
# ── API (for doorman agent API mode) ──────────────────────

# Your doorman.com API key. Can also be set via DOORMAN_API_KEY.
...
```yaml
# config.yaml — no monitored_queues means auto-discover from workers
...
Doorman redacts passwords from all log output: `redis://***@prod-redis:6379/0`.
...
# production
REDIS_URL=redis://prod-redis:6379/0 doorman audit --config prod.yaml
```
