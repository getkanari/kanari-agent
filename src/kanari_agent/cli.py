...
def _probe_redis(url: str) -> bool:
    """Returns True if Redis at url responds to ping."""
    try:
...
def cmd_init(args: argparse.Namespace) -> None:
    ...
    # Inherit URLs from environment if available
    redis_url = (
        os.environ.get("REDIS_URL")
        or os.environ.get("KANARI_REDIS_URL")
        or "redis://localhost:6379/0"
    )
    broker_url = (
        os.environ.get("CELERY_BROKER_URL")
        or os.environ.get("KANARI_CELERY_BROKER_URL")
        or redis_url
    )
...
"""
    ...
    if _probe_redis(redis_url):
        print(f"   Redis detected at {redis_url}")
    else:
        print(f"   ⚠️  Redis not reachable at {redis_url}")
        print("      Update redis_url in the config file before running.")
