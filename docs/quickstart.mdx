...
If Redis is on localhost with default settings, skip this — those are already the defaults.
...
This checks that the required libraries are installed, Redis is reachable, and your Celery workers are responding. If anything is wrong, it tells you exactly how to fix it.

If everything is green, move on. If not, fix what `doctor` reports before running `audit`.
