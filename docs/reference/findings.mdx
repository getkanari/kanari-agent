...
description: Every finding Doorman can raise, with causes and fixes
---
...
Doorman cannot connect to Redis. No queue metrics are available.
...
The oldest task in a queue has been waiting longer than `max_wait_time_seconds`. Requires [DoormanStampPlugin](/guides/latency-tracking) for accurate measurement.
...
Queues have tasks pending but no timestamps are present in the messages. Doorman cannot measure how long tasks have been waiting, so `QUEUE_SLA_BREACH` detection is blind.

**Cause:**
Celery + Redis doesn't add timestamps to queue messages by default. `task_send_sent_event=True` does not help — it emits events to the Celery event stream, not to the queue messages that Doorman reads.
