...
✅ Created kanari.yaml
   ...
   Next steps:
   kanari doctor        # verify connectivity first
   kanari audit         # one-shot health check
```
...
Checks that the required libraries are installed, Redis is reachable, and your Celery workers are responding. If anything is wrong, it tells you exactly how to fix it. Automatically loads `kanari.yaml` if it's in the current directory.
